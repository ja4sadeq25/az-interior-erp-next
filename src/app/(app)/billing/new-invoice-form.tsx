...
export default function NewInvoiceForm({ projects }: { projects: { id: string; code: string; title: string }[] }) {
  ...
  if (!open) return <button className="btn" onClick={() => setOpen(true)}>{t.billing.newInvoice}</button>;

  return (
    <form action={submit} className="card space-y-4 p-5">
      <h3 className="font-semibold">{t.billing.newTitle}</h3>
      <div className="grid gap-3 sm:grid-cols-3">
        <div><label className="label">{t.billing.clientNameReq}</label><input name="client_name" className="input" required /></div>
        <div>
          <label className="label">{t.billing.project}</label>
          <select name="project_id" className="input">
            <option value="">{t.common.none}</option>
            {projects.map((p) => <option key={p.id} value={p.id}>{p.code} · {p.title}</option>)}
          </select>
        </div>
        <div><label className="label">{t.billing.milestone}</label><input name="milestone_title" className="input" placeholder={t.billing.milestonePh} /></div>
        <div><label className="label">{t.billing.clientEmail}</label><input name="client_email" className="input" type="email" /></div>
        <div><label className="label">{t.billing.clientAddress}</label><input name="client_address" className="input" /></div>
        <div><label className="label">{t.billing.dueDate}</label><input name="due_date" className="input" type="date" /></div>
        <div><label className="label">{t.billing.vat}</label><input name="tax_rate" className="input" type="number" min="0" step="0.5" defaultValue="5" /></div>
        <div><label className="label">{t.billing.discount}</label><input name="discount" className="input" type="number" min="0" defaultValue="0" /></div>
      </div>

      <div>
        <label className="label">{t.billing.lineItems}</label>
        <div className="space-y-2">
          {rows.map((r, i) => (
            <div key={i} className="grid grid-cols-12 items-center gap-2">
              <input className="input col-span-6" placeholder={t.billing.descPh} value={r.description}
              ...
              <input className="input col-span-2" type="number" min="0" placeholder={t.billing.ratePh} value={r.rate}
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <p className="text-sm font-bold">{t.billing.subtotal}{subtotal.toLocaleString()}</p>
        </div>
      </div>

      {err && <p className="alert-error">{err}</p>}
      <div className="flex gap-2">
        <button className="btn" disabled={pending}>{pending ? t.billing.creating : t.billing.createInvoice}</button>
