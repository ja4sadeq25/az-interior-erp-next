...
export default function NewPoForm({ vendors, projects }: {
  ...
  if (!open) return <button className="btn" onClick={() => setOpen(true)}>+ New Purchase Order</button>;
  ...
  return (
    <form action={submit} className="card space-y-4 p-5">
        ...
        <div className="space-y-2">
          {rows.map((r, i) => (
            <div key={i} className="grid grid-cols-12 items-center gap-2">
              <input className="input col-span-4" placeholder="Item name" value={r.itemName}
                onChange={(e) => setRows(rows.map((x, j) => j === i ? { ...x, itemName: e.target.value } : x))} />
              <input className="input col-span-2" placeholder="Category" value={r.category}
              ...
              <input className="input col-span-1" placeholder="unit" value={r.unit}
                onChange={(e) => setRows(rows.map((x, j) => j === i ? { ...x, unit: e.target.value } : x))} />
              <input className="input col-span-2" type="number" min="0" placeholder="Unit ৳" value={r.unitPrice}
      ...
      <div className="flex gap-2">
        <button className="btn" disabled={pending}>{pending ? "Creating…" : "Create PO"}</button>
