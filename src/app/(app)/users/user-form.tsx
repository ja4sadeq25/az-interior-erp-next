  ...
  if (!open) {
    return (
      <div className="flex items-center gap-3">
        <button className="btn" onClick={() => setOpen(true)}>+ Add Staff / Client User</button>
        {ok && <span className="text-sm text-emerald-700">User created ✓</span>}
      </div>
    );
  }
  ...
  return (
    <form action={submit} className="card space-y-3 p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div><label className="label">Temp Password * (min 8)</label><input name="password" className="input" required minLength={8} /></div>
        <div>
        ...
        <div><label className="label">Title</label><input name="title" className="input" placeholder="Senior Architect" /></div>
        <div><label className="label">Department</label><input name="department" className="input" /></div>
        {role === "client" && (
          <div className="sm:col-span-2">
            <label className="label">Bound Project (client sees only this)</label>
      ...
      <div className="flex gap-2">
        <button className="btn" disabled={pending}>{pending ? "Creating…" : "Create User"}</button>
